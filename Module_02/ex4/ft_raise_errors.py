class PlantError(Exception):
    ...
    def plant_valid_name(plant:str):
        if type(plant) is not str:
            raise PlantError(f"Error: Plant name cannot be empty!")
        else:
    ...
    def plant_valid_water(water:int):
        if water > 10:
            raise PlantError(f"Error: Water level {water} is too high (max 10)")
        else:
    ...
    def plant_valid_sunlight(hours:int):
        if hours < 2:
            raise PlantError(f"Error: Sunlight hours {hours} is too low (min 2)")
        else:
