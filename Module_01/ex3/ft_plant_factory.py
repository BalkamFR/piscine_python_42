class Garden_Data_Organizer:
	...
	def print_data_plant(self):
		print(f"{self.name_plant}: {self.starting_height_plant}cm, {self.starting_age_plant} days old")


def add_plant_tab_plants(all_plants:list, name_plant_add:str, height_plant_add:int, age_plant_add):
	plant = Garden_Data_Organizer(name_plant_add, height_plant_add, age_plant_add)
	print(f"Created: {name_plant_add} ({height_plant_add}cm, {age_plant_add}, days)")
