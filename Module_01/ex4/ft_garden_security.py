class Garden_Security:
	...
	def set_height(self, height_update:int)->int:
		...
		if (height_update < 0):
			...
			print(f"Invalid operation attempted: height {height_update}cm [REJECTED]")
	...
	def set_age(self, age_update:int)->int:
		...
		if (age_update < 0):
			...
			print(f"Invalid operation attempted: age {age_update}cm [REJECTED]")
	...
	def get_info(self):
		return(f"Current plant: {self.__name_plant} ({self.get_height()}cm, {self.get_age()} days)")
	
	def print_data_plant(self):
		if (self.__starting_height_plant == -1 or self.__starting_age_plant == -1):
