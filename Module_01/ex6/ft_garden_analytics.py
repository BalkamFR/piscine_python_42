class Plant:
    ...
    def set_height(self, height_update: int) -> int:
        ...
        if (height_update < 0):
            ...
            print(
                f"Invalid operation ({self.__name_plant}) attempted: height {height_update}cm")
...
class FloweringPlant(Plant):
    ...
    def print_data_plant(self) -> str:
        print(
            f" - {self.get_name()}: {self.get_height()}cm {self.get_color_plant()} ({self.get_blooming_plant()})")
...
class GardenManager:
    ...
    class GardenStats:
        ...
        def print_stats(self):
            print(
                f"Plants added: {len(self.all_plants)}, Total growth: {self.total_grow}cm")
