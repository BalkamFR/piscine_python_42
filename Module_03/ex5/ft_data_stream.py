...
class GameData:
    all_event: list[str] = ['leveled up', 'found treasure', 'killed monster']
    all_players_found_treasure: int = 0
    all_players_killed: int = 0
    all_players_level_up: int = 0

    random.shuffle(all_event)
    all_players: list["GameData"] = []

    def __init__(self, players_name: str) -> None:
    ...
    def apply_event_players(self, event: str, index: int) -> None:
        self.event = event
    ...
    @classmethod
    def generate_events_random(cls) -> Generator[int, None, None]:
        nbr_event: int = 1

        while True:
            GameData.all_players[0].apply_event_players(
                GameData.all_event[0],
                nbr_event
            )
            nbr_event += 1
            yield nbr_event

            GameData.all_players[1].apply_event_players(
                GameData.all_event[1],
                nbr_event
            )
            nbr_event += 1
            yield nbr_event

            GameData.all_players[2].apply_event_players(
                GameData.all_event[2],
                nbr_event
            )
            nbr_event += 1
            yield nbr_event
    ...
    def generate_event() -> None:
        ...
        game: Generator[int, None, None] = GameData.generate_events_random()
        ...
        print(
            f"Treasure events: "
            f"{GameData.all_players_found_treasure}"
        )
        print(
            f"Level-up events: "
            f"{GameData.all_players_level_up}\n"
        )
...
def generator_demonstration() -> None:
    ...
    i: int = 0
    fibo: Generator[int, None, None] = fibonacci()
